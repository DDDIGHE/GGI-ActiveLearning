...
from ..utils import get_average_edge_per_molecule, get_average_edge_node_per_molecule, get_average_node_per_molecule, disable_dropout, disable_dropouta
...
class run():
    ...
    def run(self,wandb, device, train_dataset, valid_dataset, test_dataset, model,
       ...
       #define optimizer and scheduler for lossnet here
        ...
        if not os.path.exists(f'runs/{method}/run{expt}/res.txt'):
            with open(os.path.join(f'runs/{method}/run{expt}', f'res.txt'), 'w') as f:
                f.write(f'CYCLE: {cycle}   BEST:: valid:{best_valid} test_v: {best_test} test_o:{best_test_only}\n')
                
        with open(os.path.join(f'runs/{method}/run{expt}', f'res.txt'), 'a') as f:
            f.write(f'CYCLE: {cycle}   BEST:: valid:{best_valid} test_v: {best_test} test:{best_test_only}\n')
    ...
    def train(self, model, optimizer, train_loader, energy_and_force, p, loss_func, device, 
        ...
        for step, batch_data in enumerate(tqdm(train_loader)):
            ...
            edge_index, feature_dict, out, _ = model(batch_data)
            edge_features = feature_dict['edge_features']
            node_features = feature_dict['node_features']

            # print('output shape: ', out.shape)
            # print('bd y shape', batch_data.y.shape)
            # print('bd y unsqueeze shape', batch_data.y.unsqueeze(1).shape)
            ...
            if energy_and_force:
                ...
                # abs_diff = torch.abs(out - batch_data.y.unsqueeze(1))
                # loss = torch.mean(torch.abs(abs_diff))
                loss = loss_func(out, batch_data.y.unsqueeze(1)) 
                # print('Loss without adding lossnet loss: ', loss)
                # print(loss)
                # print(torch.mean(torch.abs(out - batch_data.y.unsqueeze(1))))
                # print('absolute error shape: ',torch.abs(out - batch_data.y.unsqueeze(1)).shape)

            total_loss = loss
            
            # features_list = []
            # for e_feature, n_feature in zip(edge_features, node_features):
            #     # features_list.append(get_average_edge_per_molecule(batch_data, edge_index, feature[1], device))
            #     features_list.append(get_average_edge_node_per_molecule(batch_data, edge_index, e_feature[1], n_feature, device))
            ...
            if method == 'lloss':
                ...
                # for e_feature, n_feature in zip(edge_features, node_features):
                #     av_e_n = get_average_edge_node_per_molecule(batch_data, edge_index, e_feature[1], n_feature, device)
                #     features_list.append(av_e_n)
                ...
                # print('features_list 0: ', features_list[0].shape)
                ...
                # print('llne: ',lossnet_out.shape)
                # print(torch.abs(out - batch_data.y.unsqueeze(1)).view(out.size(0)).shape)

                # print('out shape: ', out.shape)
                # print('batch_data.y shape: ', batch_data.y.unsqueeze(1).shape)

                # print('lnet out', lossnet_out)
                # import sys
                # sys.exit()
                # lossnet_loss = lossnet_loss_func(lossnet_out, abs_diff)
