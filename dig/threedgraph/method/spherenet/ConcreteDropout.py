...
class ConcreteDropout(nn.Module):
    def __init__(self, layer, weight_regularizer=1e-6, dropout_regularizer=1e-5, init_min=0.1, init_max=0.5):
        ...
        #self.p_logit = torch.nn.Parameter(torch.empty(1).uniform_(self.init_min, self.init_max)).to(device)
        #self.p_logit = torch.nn.Parameter(torch.empty(1).uniform_(self.init_min, self.init_max)).to(device).requires_grad_(True)
    ...
    def concrete_dropout(self, x):
        ...
        #print(torch.sigmoid(self.p_logit))
        return x
